...
def classify_permission_approval_ui_reply(answer_text: str) -> tuple[str, str]:
    ...
    if lowered in {"cancel", "skip", "dismiss", "건너뛰기", "취소"}:
        return ("cancel", "")
    if normalized == "3":
        raise RuntimeError("Option 3 needs a decline message. Send the reason text itself.")
    if normalized:
...
def command_ask(args: argparse.Namespace) -> int:
    ...
    if busy_state != "idle" and not args.force_while_busy and (
        use_sidecar or not args.ipc
    ):
        raise RuntimeError(describe_thread_busy_state(busy_state))
    ...
    try:
        if use_sidecar:
            ...
            sidecar_result = start_turn_via_sidecar(
                thread,
                prompt,
                timeout_sec=10.0,
            ...
            if delivered_thread is None:
                ...
                print(
                    "Codex sidecar reported success, but local session recording was not confirmed before the deadline."
                )
                print("Continuing to watch for the next Codex reply.")
            elif delivered_thread.id != thread.id:
                raise RuntimeError(
                    "Prompt landed in a different thread after sidecar delivery. "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ask_parser.add_argument(
        ...
        dest="sidecar",
        action="store_true",
        help="Send the prompt through Codex app-server turn/start for an explicit thread.",
    )
    ask_parser.add_argument(
        "--ipc-recover-ui",
        action="store_true",
