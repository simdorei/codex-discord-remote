"""Shared user-root scope for ordinary Discord command surfaces."""
...
def load_gpt_registered_thread_ids(
    db_path: Path,
    *,
    load_ids: LoadGptThreadIds = (
        gpt_registration_store.load_gpt_registered_thread_ids_read_only
    ),
) -> frozenset[str]:
    """Return every GPT-owned Codex ID, including inactive lifecycle rows."""
...
def exclude_gpt_registered_threads(
    ...
    *,
    ...
    gpt_thread_ids = load_gpt_registered_thread_ids(
        db_path,
        load_ids=load_ids,
    )
...
def load_ordinary_user_root_threads(
    ...
    *,
    ...
    """Load state DB user roots minus IDs registered in the GPT mirror DB."""
