...
BotContraT = TypeVar(
    "BotContraT", bound=discord_prefix_dispatch.PrefixDispatchBot, contravariant=True
)
...
class PrefixDispatchRuntimeDeps(Generic[BotT, MessageableT, HistoryChannelT]):
    ...
    require_messageable: Callable[
        [discord_prefix_dispatch.PrefixDispatchChannel], MessageableT
    ]
    build_doctor_message_with_history: DoctorMessageBuilder[BotT, HistoryChannelT]
    require_history_channel: Callable[
        [discord_prefix_dispatch.PrefixDispatchChannel], HistoryChannelT
    ]
...
def make_prefix_dispatch_deps(
    ...
    *,
    ...
    async def build_doctor_message(
        message: discord_prefix_dispatch.PrefixDispatchMessage,
    ) -> str:
    ...
    async def run_doctor_bridge(
        ...
        return await deps.run_bridge_and_send(
            deps.require_messageable(target), ["doctor"], "Doctor"
        )
    ...
    return discord_prefix_dispatch.build_prefix_dispatch_deps(
        discord_prefix_dispatch.PrefixDispatchFactoryDeps(
            ...
            make_prefix_gpt_deps=deps.make_prefix_gpt_deps,
